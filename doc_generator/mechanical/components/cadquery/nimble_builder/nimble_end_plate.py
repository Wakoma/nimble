...
def create_end_plate(width, depth, height):
    ...
    rail_length = width - nimble_builder.beam_width - hole_countersink_dia
    rail_offset = (width - nimble_builder.beam_width) / 2

    # Make the main body
    plate = cad.make_box(width, depth, height, center="XY")
    plate.fillet("|Z", nimble_builder.corner_fillet)

    # add star pattern to save material
    cutout = cad.make_sketch()
    cutout.add_rect(width - 2 * nimble_builder.beam_width, depth - 2 * nimble_builder.beam_width)
    for i in range(4):
    ...
    hole_positions = cad.pattern_rect(width - nimble_builder.beam_width, depth - nimble_builder.beam_width)
